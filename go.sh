...
;设置分组标志位
custom_proxy_group=🔰 节点选择`select`[]✈ 手动选择`[]✈ 延迟最低`[]✈ 故障切换`[]DIRECT`(公告|地址)
custom_proxy_group=📲 香港5倍`select`[]🔰 节点选择`[]✈ 手动选择`[]✈ 延迟最低`[]✈ 故障切换`(香港HKT-5倍)
custom_proxy_group=📹 香港1倍`select`[]🔰 节点选择`[]✈ 手动选择`[]✈ 延迟最低`[]✈ 故障切换`(香港HKT-A|香港HKT-B|香港HGC)
custom_proxy_group=🎥 台湾`select`[]🔰 节点选择`[]✈ 手动选择`[]✈ 延迟最低`[]✈ 故障切换`(台湾)
custom_proxy_group=📺 美国`select`[]🔰 节点选择`[]✈ 手动选择`[]✈ 延迟最低`[]✈ 故障切换`(美国)
custom_proxy_group=📺 日本`select`[]🔰 节点选择`[]✈ 手动选择`[]✈ 延迟最低`[]✈ 故障切换`(日本)
custom_proxy_group=📺 新加坡`select`[]🔰 节点选择`[]✈ 手动选择`[]✈ 延迟最低`[]✈ 故障切换`(新加坡)
